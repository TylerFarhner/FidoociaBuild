...
import { Text, Alert, View, StyleSheet, SafeAreaView, Dimensions, ScrollView } from "react-native";
...
function RegisterScreen() {
  ...
  const register = () => {
    ...
    if (!isValid) {
      Alert.alert("Incomplete Profile","Please make sure all fields are filled out properly");
    } else {
      ...
      .catch(error => {
        var emailInUse = "The email address is already in use by another account.";
        if (error.message == emailInUse) 
        {
          Alert.alert("Email in Use", "There is already an account created using this email address. Try signing in or using a different email address.")
        } else 
        {
