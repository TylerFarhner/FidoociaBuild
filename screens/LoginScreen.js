...
function LoginScreen({ navigation }) {
  ...
  const signIn = () => {
    auth.signInWithEmailAndPassword(email, password).catch((error) => {
      ...
      var noAccountErrorMessage = "There is no user record corresponding to this identifier. The user may have been deleted."
      if (errorMessage == noAccountErrorMessage) {
        Alert.alert("Whoops!", "We don't have record of that account. \n\nCheck that you typed everything correctly, or register if you don't have an account yet :)")
      } else {
        alert(errorMessage);
      }
