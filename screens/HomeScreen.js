...
import { auth, fire, db } from "../backend/firebase";
...
function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    console.log("TODAY IS", todayDate, selectedDay.dateString);

    db.collection('users').doc(auth.currentUser.uid).collection('dates').get()
      .then((querySnapshot) => {
          ...
          <Card>
            <Card.Content>
              ...
              >
                ...
                >
                  <Text style={{ marginBottom: height / 100 }}>{item.startTime}</Text>
                  <Text style={{ fontSize: width / 23, marginBottom: height / 250 }}>
                  ...
                  <TouchableOpacity onPress={() => navigation.navigate("CheckOutScreen", {
                    ...
                    style={{ backgroundColor: 'green', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: height / 75, paddingHorizontal: width / 30, paddingVertical: height / 100, borderRadius: width / 2 }}>
                    <MaterialCommunityIcons name={"message-text-clock"} size={width / 25} color={'white'} />
                    <Text style={{ fontWeight: 'bold', color: 'white', marginLeft: width / 50, fontSize: width / 35 }}>{'SHARE DATE'}</Text>
                  </TouchableOpacity>
                </View>
                <TextAvatar
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <TouchableOpacity onPress={() => navigation.navigate('Settings')} style={{alignItems: 'center', justifyContent: 'center', width: width/8, height: width/8}}>
          <Ionicons
